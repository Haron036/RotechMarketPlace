...
const PaymentIcons = {
  FLUTTERWAVE: () => (
    <svg viewBox="0 0 60 25" className="h-5" fill="none">
      <circle cx="12" cy="12" r="8" fill="#F5A623" />
      <circle cx="28" cy="12" r="8" fill="#FF6B35" opacity="0.85" />
      <circle cx="44" cy="12" r="8" fill="#E8232A" opacity="0.7" />
    </svg>
  ),
  PAYPAL: () => (
...
const Cart = () => {
  ...
  const paymentMethods = [
    { id: "FLUTTERWAVE", label: "Card Payment",  desc: "Visa, Mastercard, Amex" },
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (selectedMethod === "FLUTTERWAVE") {
        ...
        toast({ title: "Redirecting to payment...", description: "You will be redirected shortly." });
        localStorage.setItem("flw_order_id", data.orderId);
        clearCart();
        setTimeout(() => { window.location.href = data.paymentLink; }, 1000);
      }
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <div className="flex items-center border border-border rounded-md">
                    ...
                    <button onClick={() => updateQuantity(item.product.id, item.quantity + 1)}
                      disabled={item.quantity >= (item.product.stock ?? 99)} className="px-2 py-1 text-xs hover:bg-secondary disabled:opacity-40">+</button>
                  </div>
                </div>
              </motion.div>
            ))}
          ...
          <div className="lg:col-span-1">
            <div className="rounded-xl bg-card border border-border p-6 sticky top-24 space-y-4">
              ...
              <div className="space-y-2 text-sm border-t border-border pt-3">
                ...
                <div className="border-t border-border pt-2 flex justify-between font-semibold text-foreground"><span>Total</span><span>{formatPrice(totalPrice)}</span></div>
              </div>
              <Button onClick={() => setShowPaymentModal(true)} className="w-full marketplace-gradient border-0 text-primary-foreground" size="lg">
                <CreditCard className="w-4 h-4 mr-2" /> Proceed to Checkout
              </Button>
              <p className="text-[10px] text-center text-muted-foreground">Secure checkout · Buyer protection included</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }}
              ...
              <AnimatePresence>
                {selectedMethod === "MPESA" && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="mb-6 overflow-hidden">
                    <label className="text-xs font-medium text-foreground block mb-1.5">M-Pesa Phone Number</label>
                    <input type="tel" placeholder="e.g. 0712345678" value={mpesaPhone}
                    ...
                    <p className="text-[10px] text-muted-foreground mt-1">You will receive an STK push prompt on this number.</p>
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <div className="flex gap-3">
                <Button variant="outline" className="flex-1" onClick={() => setShowPaymentModal(false)} disabled={checkingOut}>Cancel</Button>
                <Button className="flex-1 marketplace-gradient border-0 text-primary-foreground" onClick={handleCheckout} disabled={checkingOut || !selectedMethod}>
                  {checkingOut ? (
              ...
              <p className="text-[10px] text-center text-muted-foreground mt-3">🔒 Secured by 256-bit SSL encryption</p>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
